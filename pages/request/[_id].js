...
export default function StudentDetails({ student }) {
    ...
    const closeStudent = async (e) => {
        ...
        const res = await fetch('/api/student/close', {
            ...
            body: JSON.stringify({ _id: student?._id, owner: true, closed: true, closedReason: reason || 'Not Mentioned' })
        })
        const response = await res.json()
        if (res.status == 200) {
            toast.success(response.msg);
        ...
        <>
            ...
            <div className='text-gray-800 font-semibold text-base sm:text-lg max-w-3xl mx-auto p-5 sm:px-10 border rounded-lg shadow mt-5 mb-10'>
                ...
                <div className='space-y-2 py-4'>
                    ...
                    {student?.requestEmail && <h1>Reason for Email: <span className='info__text'>{student?.emailReason} Lorem ipsum dolor sit amet consectetur adipisicing elit. Architecto, hic. </span></h1>}
                    {student?.internetReason && < h1 > Reason for Internet: <span className='info__text'>{student?.internetReason} Lorem ipsum dolor, sit amet consectetur adipisicing elit. Unde dolorem enim praesentium, non illo exercitationem porro reprehenderit delectus. Suscipit delectus quam perferendis libero? Impedit quidem nesciunt aspernatur natus libero nostrum, ipsam doloremque provident maiores vitae deserunt iusto architecto dolor nam culpa hic porro autem! Aliquid ipsa cum eveniet iure harum. </span></h1>}
                ...
                <div className='flex items-center justify-center gap-5 my-5'>
                    ...
                    {!student?.isVerified && <button className="px-5 py-2 text-lg font-medium text-white hover:bg-purple-800 border bg-purple-600 rounded-3xl transition-all duration-150 ease-out cursor-pointer" onClick={() => setIsVerify(true)} >Verified</button>}
                    <button className="px-5 py-2 text-lg font-medium text-white hover:bg-purple-800 border bg-purple-600 rounded-3xl transition-all duration-150 ease-out cursor-pointer" onClick={() => setIsClose(true)} >Close</button>
            ...
            {
                isClose && <div className='fixed top-0 left-0 w-full h-full min-h-screen bg-gray-50 bg-opacity-20 backdrop-filter backdrop-blur grid place-items-center z-10'>
                    <div className='max-w-xl rounded-xl shadow-lg border border-gray-100 p-5 sm:p-10 bg-white flex flex-col justify-center items-center'>
                        ...
                        <div className='flex items-center justify-around gap-x-5'>
                            <button className="px-5 py-2 text-lg font-medium text-white hover:bg-purple-800 border bg-purple-600 rounded-3xl transition-all duration-150 ease-out cursor-pointer"
                                onClick={() => closeStudent()} disabled={isSending}>{isSending ? 'Wait...' : 'Set Closed'}</button>
                            <button className="px-5 py-2 text-lg font-medium text-white hover:bg-purple-800 border bg-purple-600 rounded-3xl transition-all duration-150 ease-out cursor-pointer"
                                onClick={() => setIsClose(false)} disabled={isSending}>Cancel</button>
                        </div>
                    </div>
                </div>
            }
